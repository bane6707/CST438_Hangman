...
//  ID: bane6707
//  Date: 3/9/18.
//  Abstract: Utility functiona page
**/
...
/*
    ...
    Parameters: Maximum attempts, no. of attempts made by user,
                Hidden string and the answer.
    Return values: Returns a character array.
*/
    function checkGameOver($MAX_ATTEMPTS,$userAttempts, $answer, $hidden)
    {
        if ($userAttempts >= $MAX_ATTEMPTS)
            {
              echo '<tr><td><font color="red">Game Over.';
              echo "The correct word is indeed <b>";
              foreach ($answer as $letter) echo $letter;
              echo '</b></font></td></tr><form action = "" method = "post">';
              echo '<tr><td align="center"><input type = "submit"';
              echo 'name = "newWord" value = "Try another Word"/>';
              echo '</form></td></tr>';
                die();
            }
            if ($hidden == $answer)
            {
                echo '<tr><td><font color="green">Game Over.';
                echo "The correct word is indeed <b>";
                foreach ($answer as $letter) echo $letter;
                echo '</b></font></td></tr><form action = "" method = "post">';
                echo '<tr><td align="center"><input type = "submit"';
